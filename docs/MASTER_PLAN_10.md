...
- ✅ (P7) **SCORECARD generator** regenerated by verify like STATUS — build-completeness
  9.7/10 (honest proxy view; the one red measured gate is product coverage 73% < 80%).
...
- ✅ (P6/coverage) product-surface tests for cli.ts (0→20%) + farm-service authoring loop
  + destination usefulness — global line coverage 73→75.7% (floor ratcheted to 75%).
- ⬜ THE remaining 10/10 gate is **product line coverage 75.7% → 80%**. The pure-function
  surface is largely covered; closing it needs the **research-cluster quarantine** (move
  the ~7,600-line calibration code to src/research/ + a 2nd boundary rule + exclude from
  product coverage) OR browser/IO-heavy tests (browser-pool 52%). Quarantine is the plan's
  intended path and the prerequisite product-surface tests now exist.
- ⬜ also remaining: P4 recipe canary + coverage-report matrix; P6 Biome lint gate +
  evidence-runner split; P7 publish metadata + tarball test + conformance suite.
