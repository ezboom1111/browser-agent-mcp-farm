...
- ✅ (P4/observability) per-run **metrics.json** (p50/p95 stage latency).
- ✅ (P5/trust) **tamper-evident hash-chained decision log** (verify-decision-log).
- ✅ (P5/portability, HEADLINE) **self-contained `.evb` archive** — verifies FULLY OFFLINE
  (proven by a test that deletes the runDir first); signed Ed25519 + Merkle.
- ✅ (P6/lifecycle) **retention** (purge-run / prune-runs).
- ✅ (P7) **SCORECARD generator** regenerated by verify like STATUS — build-completeness
  **10/10** (all 8 domains green; honest proxy view, not a holistic quality grade).
- ✅ (P5/trust) worked **agent-to-agent `.evb` exchange** example (A signs → B verifies
  offline, trusting hashes not the producer; rejects MITM tamper + impostor key).
- ✅ (P6/coverage) **product line coverage 76.2% → 80.4%** — the last 10/10 gate, closed
  with ~130 in-process cli/farm-service/claim-gate/browser-pool tests behind a shared
  harness (NOT the research-cluster quarantine, which would have *lowered* the reported
  number; the honest path was to add real product-surface tests). Floor ratcheted to 80.
- ✅ (P4) **`recipe-canary`** (golden-vs-live selector health, auto-demote) + **`coverage-report`**
  classifier (autonomous_ready / api_backed / headed_only / blocked / unmaintained, gated on a
  fresh passing canary inside the named maintenance budget).
- ✅ (P7) **Apache-2.0 license** + `LICENSE` + author; **`test:tarball`** packaged-artifact
  gate (manifest + packed bin runs end-to-end); **parallel-safety conformance suite**
  (global context cap → `capacity_exhausted` backpressure, reaper crash-recovery, redaction,
  same-port refusal, **real two-process** O_EXCL profile-lock contention).
- ✅ (P6) **Biome lint+format gate** (`npm run lint`, wired into verify) + **`evidence-runner.ts`
  split** (3064 → 2689 lines: text helpers + data-model modules extracted).
- ⬜ beyond the measured 10/10 (optional polish): P3 microdata/RDFa parser + capture-provenance
  attestation (HAR); P4 `recover-slot` headed loop; P7 nightly quota-capped canary + output
  accessibility.
