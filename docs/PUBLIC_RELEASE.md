...
## 1. 라이센스: 소스공개 — 열람·평가 전용 (2026-08-25 전환)

> **결정 변경 (2026-08-25, 사용자)**: "공개지 가져가서 쓰면 안된다" — 공개는
> 포트폴리오 **열람**용이며, 제3자에게 사용·복제·수정·재배포권을 부여하지
> 않는다. 이에 따라 기존 Apache-2.0(2026-05 결정)를 폐기하고 커스텀
> **Source-Available License (View / Evaluation Only)** 로 전환했다.
> `LICENSE` + `NOTICE` + `package.json`의 `"license": "SEE LICENSE IN LICENSE"`
> 로 적용됨.

### 새 라이센스가 허용/금지하는 것

- **허용(무허가)**: GitHub에서의 열람·검색·플랫폼 내 포크, 그리고 저자의
  작업을 평가하기 위한 **무수정 로컬 설치·실행**.
- **금지(서면 허가 필요)**: 제품/서비스/연구 등 어떤 프로젝트에서의 사용,
  복제·수정·파생물 제작, 재배포·재라이센스, ML 학습 데이터 사용.
- 팀원 등 허가받은 사람은 저자의 서면 허가(이메일이면 충분)로 사용 가능 —
  허가는 라이센스 밖에서 개별 부여한다.

### 정직한 한계 (알고 있어야 하는 것)

1. **GitHub 규약상 공개 저장소의 열람·플랫폼 내 포크 자체는 막을 수 없다.**
   라이센스가 막는 것은 그 코드의 *사용·재배포 권리*다.
2. **이미 배포된 Apache-2.0 사본은 소급 취소 불가**: npm의 0.6.1·0.7.0, 그리고
   refcap이 2026-08-25 약 2.5시간 동안 Apache-2.0로 노출됐던 스냅샷. 그 시점에
   취득된 사본은 그 사본에 한해 Apache-2.0 권리가 유지된다. 이후 버전(0.8.0+)
   은 전부 새 라이센스.
3. 이 커스텀 라이센스는 OSI 정의상 "오픈소스"가 아니다 — 문서·마케팅에서
   "open-source"라는 표현을 쓰지 않는다(source-available로 표기).

### 저작권 표기

- `LICENSE`: Source-Available License 전문 + 한국어 요약, `Copyright 2026 이지범`
- `NOTICE`: 패키지명 + 저작권 + 런타임 의존성 라이센스 요약 + ≤0.7.0 Apache 이력
