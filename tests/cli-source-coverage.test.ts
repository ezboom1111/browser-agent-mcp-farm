...
import { buildSourceCoverageReadinessAudit, buildSourceCoverageReadinessRetryPlan } from "../src/source-coverage-readiness.js";
...
function promotionGroup(platform: SourcePlatform, sourceFamily: SourceFamily, status: SourceNavigationPromotionSummary["groups"][number]["status"], actionCount: number, summary: Partial<SourceNavigationPromotionSummary["groups"][number]["catalogSummary"]>): SourceNavigationPromotionSummary["groups"][number] {
  return {
    platform,
    sourceFamily,
...
describe("source-coverage-calibrate (in-process, plan-only / dry-run / arg validation)", () => {
  it("--plan-only writes offline plan files and reports plan_only mode (ok=true)", async () => {
    ...
    dirs.push(runRoot);
    const { out, exitCode } = await runCli(["source-coverage-calibrate", "--category", "search", "--locale", "ko-KR", "--query", "seoul hotel", "--plan-only", "--run-root", runRoot]);
  ...
  it("--plan-only --fail-not-ready exits 1 when actionable slots are not ready", async () => {
    ...
    dirs.push(runRoot);
    const { out, exitCode } = await runCli(["source-coverage-calibrate", "--category", "search", "--locale", "ko-KR", "--plan-only", "--fail-not-ready", "--run-root", runRoot]);
    expect(out).toContain('"mode": "plan_only"');
  ...
  it("--dry-run produces the same offline plan_only output as --plan-only", async () => {
    ...
    dirs.push(runRoot);
    const { out, exitCode } = await runCli(["source-coverage-calibrate", "--platform", "naver_search", "--dry-run", "--run-root", runRoot]);
    expect(out).toContain('"mode": "plan_only"');
    expect(exitCode).toBeFalsy();
  });

  it("rejects --calibration-concurrency > 1 with --persistent-profile (arg validation throw)", async () => {
    const { out, exitCode } = await runCli(["source-coverage-calibrate", "--category", "search", "--calibration-concurrency", "2", "--profile", "p1", "--persistent-profile", "--plan-only"]);
...
describe("source-coverage-retry-plan (in-process render/check/command branches)", () => {
  ...
  it("default json format round-trips a generated retry-plan file", async () => {
    const { out, exitCode } = await runCli(["source-coverage-retry-plan", "--retry-plan", rp, "--format", "json"]);
    expect(out).toContain('"ok": true');
  ...
  it("--format retry-commands emits only the calibrate retry command (no setup)", async () => {
    const { out, exitCode } = await runCli(["source-coverage-retry-plan", "--retry-plan", rp, "--format", "retry-commands"]);
  ...
  it("--format setup-commands emits only the auth-login setup command (no retry)", async () => {
    const { out, exitCode } = await runCli(["source-coverage-retry-plan", "--retry-plan", rp, "--format", "setup-commands"]);
  ...
  it("--platform filter + --fail-empty exits 1 on empty result", async () => {
    const { out, exitCode } = await runCli(["source-coverage-retry-plan", "--retry-plan", rp, "--platform", "daum_search", "--fail-empty", "--format", "json"]);
    expect(out).toContain('"itemCount": 0');
    expect(exitCode).toBe(1);
  });

  it("--format check --check-profiles --fail-check exits 1 when the saved profile is absent", async () => {
    const { out, exitCode } = await runCli(["source-coverage-retry-plan", "--retry-plan", rp, "--format", "check", "--check-profiles", "--fail-check"]);
    expect(out).toContain('"ok": false');
  ...
  it("errors when --retry-plan is missing", async () => {
    const { out, exitCode } = await runCli(["source-coverage-retry-plan", "--format", "json"]);
  ...
  it("rejects an invalid --priority value", async () => {
    const { out, exitCode } = await runCli(["source-coverage-retry-plan", "--retry-plan", rp, "--priority", "nope"]);
  ...
  it("--output-file writes rendered output to disk instead of stdout", async () => {
    ...
    const { out, exitCode } = await runCli(["source-coverage-retry-plan", "--retry-plan", rp, "--format", "markdown", "--output-file", outFile]);
...
describe("source-coverage-retry-plan selector-hint file checks (read-only existsSync)", () => {
  it("--check-files reports selector_hint_file_missing for an absent hint file", async () => {
    ...
    const { out, exitCode } = await runCli(["source-coverage-retry-plan", "--retry-plan", rp2, "--format", "check", "--check-files"]);
    expect(out).toContain('"ok": false');
  ...
  it("--only-check-ok drops the failing item before render", async () => {
    const missingHints = join(await makeTempDir("farm-cli-hints-"), "missing-selector-hints.tsv");
    const rp2 = await writeRetryPlanFile({ selectorHints: missingHints });
    const { out, exitCode } = await runCli(["source-coverage-retry-plan", "--retry-plan", rp2, "--only-check-ok", "--check-files", "--format", "json"]);
    expect(out).toContain('"itemCount": 0');
