...
export type BrowserOverlayDismissalKind = "cookie_consent" | "app_banner" | "newsletter_prompt" | "modal_close" | "generic_overlay";
...
export class BrowserPool {
  ...
  async capturePage(agentId: string, contextToken: string, pageId: string, captureId?: string, signal?: AbortSignal): Promise<{ records: ArtifactRecord[] }> {
    ...
    try {
      ...
      const records = await withAbort(this.artifactWriter.writeCaptureBundle(captureId === undefined ? bundleInput : { ...bundleInput, captureId }), signal);
      ...
      const records = await this.artifactWriter.recordFailure(captureId === undefined ? failureInput : { ...failureInput, captureId });
      return { records };
    }
  }
  ...
  async captureLocator(agentId: string, contextToken: string, pageId: string, selector: string, captureId?: string, signal?: AbortSignal): Promise<{ records: ArtifactRecord[] }> {
    ...
    try {
      ...
      const [html, text, screenshot] = await withAbort(Promise.all([locator.evaluate((element) => (element as HTMLElement).outerHTML).catch(() => ""), locator.innerText({ timeout: 2_000 }).catch(() => ""), locator.screenshot({ timeout: 10_000 })]), signal);
  ...
  async readClientState(agentId: string, contextToken: string, pageId: string, propertyName: string, maxJsonLength = 2_000_000, signal?: AbortSignal): Promise<BrowserClientStateResult> {
    ...
    const frameResults = await withAbort(
      Promise.all(
        frames.map(async (frame, frameIndex): Promise<BrowserClientStateFrame> => {
  ...
  async discoverLinkTargets(agentId: string, contextToken: string, pageId: string, maxLinks = 25, signal?: AbortSignal): Promise<BrowserLinkTargetsResult> {
    ...
    const frames = pageState.page.frames();
    const perFrameTargets = await withAbort(
      Promise.all(
        frames.map(async (frame, frameIndex) => {
          const baseUrl = frame.url() || currentUrl;
          try {
            const frameTargets = await frame.locator(`a, ${DESTINATION_ATTRIBUTE_SELECTOR}`).evaluateAll(
              (elements, args) => {
                ...
                const textFor = (element: Element): string => {
                  ...
                  const textParts = [element.textContent, element.getAttribute("aria-label"), element.getAttribute("title"), element.getAttribute("data-title"), element.getAttribute("data-name"), imageAlt].map((value) => (value ?? "").replace(/\s+/g, " ").trim()).filter((value) => value.length > 0);
                  return textParts[0] ?? "";
                };
                const addCandidate = (rawHref: string, element: Element, elementIndex: number, source: "anchor" | "attribute", attributeName?: string): boolean => {
                  let hrefValue = "";
                  try {
                  ...
                  try {
                    ...
                    valid = isVisible && (href.protocol === "http:" || href.protocol === "https:") && (source === "anchor" || text.length > 0) && (currentWithoutHash.length === 0 || hrefWithoutHash !== currentWithoutHash);
                  } catch {
                    valid = false;
                  }
                  candidates.push({
  ...
  async readLinkTargets(agentId: string, contextToken: string, pageId: string, selector: string, maxLinks = 10, timeoutMs = 2_000, signal?: AbortSignal): Promise<BrowserLinkTargetsResult> {
    ...
    const perFrameTargets = await withAbort(
      Promise.all(
        visibleFrames.map(async (frameInfo) => {
          const baseUrl = frameInfo.frame.url() || currentUrl;
          try {
            const frameTargets = await frameInfo.frame.locator(selector).evaluateAll(
              (elements, args) => {
                let currentWithoutHash = "";
                try {
                  const currentUrl = new URL(args.baseUrl);
  ...
  async inspectSelector(agentId: string, contextToken: string, pageId: string, selector: string, options: { maxMatches?: number; maxTextLength?: number; signal?: AbortSignal } = {}): Promise<BrowserSelectorInspection> {
    const lease = this.leaseManager.assertActive(contextToken, agentId);
    const pageState = this.getPageState(lease.contextToken, pageId);
    ...
    const frameInspections = await withAbort(
      Promise.all(
        pageState.page.frames().map(async (frame, frameIndex) => {
          const frameUrl = frame.url();
          const frameName = frame.name();
          try {
            const result = await frame.locator(selector).evaluateAll(
              (nodes, args) => {
    ...
    const matches = frameInspections.flatMap((frameInspection) =>
      frameInspection.matches.map((match) => ({
        ...match,
  ...
  async dismissBenignOverlays(agentId: string, contextToken: string, pageId: string, maxActions = 3, signal?: AbortSignal): Promise<BrowserOverlayDismissalReport> {
    const lease = this.leaseManager.assertActive(contextToken, agentId);
    const pageState = this.getPageState(lease.contextToken, pageId);
    return withAbort(runBenignOverlayDismissal(pageState.page, Math.max(0, Math.min(10, maxActions))), signal);
  }

  async scroll(agentId: string, contextToken: string, pageId: string, direction: "down" | "up" | "bottom" | "top", pixels: number, signal?: AbortSignal): Promise<{ ok: true; url: string; scrollY: number }> {
    const lease = this.leaseManager.assertActive(contextToken, agentId);
    const pageState = this.getPageState(lease.contextToken, pageId);
    const scrollY = await withAbort(
      pageState.page.evaluate(
        ({ direction: pageDirection, pixels: pagePixels }) => {
  ...
  async captureAfterIdle(agentId: string, contextToken: string, pageId: string, captureId: string | undefined, waitMs: number, idleMs: number, timeoutMs: number, signal?: AbortSignal): Promise<{ records: ArtifactRecord[] }> {
    const lease = this.leaseManager.assertActive(contextToken, agentId);
    const pageState = this.getPageState(lease.contextToken, pageId);
    if (waitMs > 0) {
      await abortableDelay(waitMs, signal);
    ...
    await withAbort(
      pageState.page.waitForLoadState("networkidle", { timeout: timeoutMs }).catch(() => undefined),
      signal
    );
    if (idleMs > 0) {
  ...
  async sampleFrames(
    ...
    const summaryRecords = await withAbort(
      this.artifactWriter.writeCaptureBundle({
        ...commonInput,
    ...
    for (const [index, timestampSec] of plan.timestampsSec.entries()) {
      ...
      const activeCues = await withAbort(
        readActiveCues(page, input.selector).catch(() => []),
        input.abortSignal
      );
      ...
      try {
        ...
        const frameRecords = await withAbort(
          this.artifactWriter.writeCaptureBundle({
            ...commonInput,
...
async function collectVisibleFrameText(page: Page, currentUrl: string): Promise<BrowserVisibleTextResult> {
  ...
  const frameResults = await Promise.all(
    frames.map(async (frame, frameIndex): Promise<BrowserVisibleTextFrameMetadata & { text: string }> => {
      const frameUrl = frame.url() || currentUrl;
      const frameName = frame.name();
      try {
        const rawText = await frame
          .locator("body")
...
async function collectVisibleLinks(page: Page, currentUrl: string, maxLinks = 25): Promise<BrowserLinkTarget[]> {
  const normalizedMaxLinks = Math.max(1, Math.min(100, maxLinks));
  const frames = page.frames();
  const perFrameCandidates = await Promise.all(
    frames.map(async (frame, frameIndex) => {
      const baseUrl = frame.url() || currentUrl;
      const frameName = frame.name();
      try {
        ...
        return candidates.map((candidate) => {
          ...
          try {
            ...
            valid = candidate.visible && (parsed.protocol === "http:" || parsed.protocol === "https:") && (candidate.source === "anchor" || candidate.text.length > 0);
          } catch {
            valid = false;
          }
          return {
...
function linkTargetsResultFromCandidates(input: {
  ...
  const selectedTargets = uniqueTargets.length >= input.normalizedMaxLinks ? uniqueTargets.slice(0, input.normalizedMaxLinks) : [...uniqueTargets, ...duplicateTargets].slice(0, input.normalizedMaxLinks);
  ...
  const report = await page.evaluate(
    async ({ maxActions: actionLimit }) => {
      ...
      function interactiveElements(): HTMLElement[] {
        return Array.from(document.querySelectorAll("button,a,[role='button'],input[type='button'],input[type='submit'],[aria-label]")).filter((element): element is HTMLElement => element instanceof HTMLElement);
      }

      function normalizedLabel(element: HTMLElement): string {
        return normalize([element.innerText, element.getAttribute("aria-label"), element.getAttribute("title"), element.getAttribute("value"), element.getAttribute("id"), element.getAttribute("class")].filter((value): value is string => value !== null && value.trim().length > 0).join(" "));
      }
      ...
      function normalizedOverlayContext(element: HTMLElement): string {
        ...
        for (let depth = 0; current !== null && depth < 5; depth += 1) {
          if (current instanceof HTMLElement) {
            values.push(current.innerText.slice(0, 500), current.getAttribute("aria-label") ?? "", current.getAttribute("role") ?? "", current.id, current.className);
          }
      ...
      function isAllowedDismissal(label: string): boolean {
        ...
        return containsAny(label, ["close", "dismiss", "not now", "no thanks", "maybe later", "skip", "reject all", "reject optional", "decline", "only necessary", "necessary only", "continue without accepting", "got it"]);
      }
      ...
      function isBlockedAction(label: string, context: string): boolean {
        ...
        return containsAny(label, ["log in", "login", "sign in", "sign up", "create account", "continue with facebook", "continue with google", "open app", "open the app", "download the app", "get the app", "continue in the app", "accept all", "allow all", "i agree", "agree and continue"]);
      }
...
async function drainMediaCaptures(state: PageState, signal?: AbortSignal): Promise<void> {
  ...
  await Promise.race([Promise.allSettled(pending), abortableDelay(MEDIA_DRAIN_TIMEOUT_MS, signal)]);
}
    ...
    candidates.push(
      await page
        .evaluate(() => {
          ...
          return [htmlElement.innerText, htmlElement.getAttribute("aria-label"), htmlElement.getAttribute("name"), htmlElement.getAttribute("id"), htmlElement.getAttribute("placeholder"), htmlElement.getAttribute("value")].filter(Boolean).join(" ");
        })
        .catch(() => "")
    );
  }
...
async function framesWithVisibleSelector(frames: Frame[], selector: string, timeoutMs: number, signal?: AbortSignal): Promise<Array<{ frame: Frame; index: number }>> {
...
function containsPaymentSignal(value: string): boolean {
  const blocked = ["checkout", "payment", "billing", "credit-card", "credit card", "card-number", "card number", "cvc", "cvv", "expiry", "pay.", "pay-", "pay_", "pay now", "결제", "카드"];
