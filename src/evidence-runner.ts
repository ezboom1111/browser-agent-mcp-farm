...
function detectedTextScriptFamilies(value: string): DestinationTextScriptFamily[] {
  const counts = countTextScripts(value);
  return TEXT_SCRIPT_FAMILIES.filter((script) => counts[script] > 0);
}

function hasDominantTextScriptMismatch(query: string, evidenceText: string): boolean {
...
function destinationQueryFromUrl(url: string): string | undefined {
  try {
...
function matchingTextTokens(query: string, value: string): string[] {
